...
class _af_design:
  ...
  def _mask_gradients(self, grad):
    # Assumes grad is a PyTree with ['seq'] key representing the logits
    if "seq" in grad:
        mask = jnp.ones_like(grad["seq"])
        for idx in self.fixed_positions:
            mask = mask.at[0,idx,:].set(0.0)  # zero-out gradients at fixed positions
        grad["seq"] *= mask
    return grad
  ...
  def step(self, lr_scale=1.0, num_recycles=None,
    ...
    self.aux["grad"] = self._mask_gradients(self.aux["grad"])  # zero out gradients at fixed positions  
